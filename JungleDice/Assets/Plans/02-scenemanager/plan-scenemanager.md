...
### `LogoSceneManager`: 씬 진입에 반응만 하는 다른 매니저와 달리 전이를 "요청"하는 특수 케이스

`LoginSceneManager` / `MainMenuSceneManager` / `InGameSceneManager`는 전부 이미 벌어진 상태 변화(`GameStateChanged`)에 반응만 한다. 반면 `Logo` 씬은 그 반대다 — 앱이 켜지자마자 `GameManager`가 `None → Logo`로 진입하지만, 그 다음 `Logo → Login` 전이를 실제로 트리거하는 주체가 아직 없다(`plan-gamemanager.md`의 `LogoSequence`는 `Logo` 진입까지만 담당).

`LogoSceneManager`가 이 역할을 맡는다: 스플래시/로고 연출, 최소 노출 시간 대기 등 **Logo 씬 전용 연출**을 담당하고, 끝나면 `GameManager.Instance.ChangeState(...)`를 직접 호출하는 대신 **`LogoSceneReady` 이벤트만 발행**한다. `GameManager`가 그 이벤트를 구독해 `ChangeState(GameState.Login)`을 호출한다.

```csharp
// LogoSceneManager — 연출이 끝나면 이벤트로만 알림
...
- `ChangeState` 호출 권한은 여전히 `GameManager` 한 곳에만 있다 — `LogoSceneManager`가 직접 `GameManager.Instance`를 참조하며 상태를 바꾸는 걸 막기 위함
- `GameManager`도 `LogoSceneManager.Instance`를 참조하지 않는다 — "씬 매니저는 전역 시스템을 직접 참조하지 않는다" 원칙이 반대 방향에도 동일하게 적용됨
- `GameState.Logo`는 `_validTransitions`상 재진입 경로가 없으므로 `LogoSceneReady`는 앱 생명주기 동안 정확히 한 번만 유효하게 발행됨 (중복 발행돼도 `GameManager.ChangeState`의 `CurrentState == next` 가드로 안전)
...
LogoSceneManager     : SceneSingleton<LogoSceneManager>       ← Logo 씬 전용 (스플래시 연출 → LogoSceneReady 발행)
...
각 씬 매니저는 서로를 모르고, 서로의 존재도 필요 없다. 오직 `EventBus`와 자기 씬 안의 오브젝트만 안다. `LogoSceneManager`만 예외적으로 `GameManager`에게 "다음으로 넘어가도 된다"는 신호(`LogoSceneReady`)를 보내지만, 이 역시 이벤트를 통해서만 이루어진다.
...
└── Scripts/
    ...
    │       └── SceneLoader.cs
    ├── Logo/
    │   └── LogoSceneManager.cs       ← 신규 (Logo 씬 전용, 예시 스켈레톤)
    ├── Login/
...
### GameManager.cs 변경 — `LogoSceneReady` 구독 추가
...
- `LogoSequence()`는 지금처럼 `None → Logo` 진입까지만 담당, `Logo → Login` 전이는 `LogoSceneReady` 수신 시 처리
- `GameEvents.cs`에 `public record LogoSceneReady();` 추가 필요
...
│   ├── GameManager.cs
│   └── SceneLoader.cs
└── LogoManagers (GameObject, 씬 로컬 — DontDestroyOnLoad 아님)
    └── LogoSceneManager.cs
...
`Logo` 씬 안에 영속 오브젝트(`GameManagers`)와 씬 로컬 오브젝트(`LogoManagers`)가 함께 있다는 점에 주의: `LogoSceneManager`는 `Login`으로 전환되는 순간 Unity가 자동으로 파괴하며, `DontDestroyOnLoad`를 호출하지 않는다. `Login` / `MainMenu` / `InGame`의 `{Scene}Managers`도 마찬가지로 씬이 바뀌면 자동 파괴된다.
...
| `LogoSceneReady`가 여러 번 발행됨 (코루틴 중복 실행 등) | `GameManager.ChangeState`의 `CurrentState == next` 가드로 두 번째 호출은 조기 반환, 부작용 없음 |
| `LogoSceneManager`가 파괴된 뒤에도 `GameManager`의 `LogoSceneReady` 구독이 남아있음 | 문제 없음 — `GameState.Logo`는 `_validTransitions`상 재진입 경로가 없어 앱 생명주기 동안 다시 발행될 일이 없음 |
...
| # | 시나리오 | 기대 결과 |
|---|----------|-----------|
| 1 | Logo 씬 시작 → `LogoSceneManager.OnAwake()` | `LogoRoutine` 코루틴 시작, `_minSplashDuration` 대기 |
| 2 | 스플래시 대기 종료 | `LogoSceneReady` 발행 → `GameManager.ChangeState(Login)` 자동 호출 → `SceneLoader`가 Login 씬 로드 |
| 3 | Login 씬 로드 | `LoginSceneManager.Instance` 할당, `OnAwake()` 호출, `LogoSceneManager`는 Logo 씬과 함께 자동 파괴 |
...
- **`LogoSceneManager`는 예외적으로 전이를 유발하지만 `ChangeState`를 직접 호출하지 않는다**: `EventBus.Publish(new LogoSceneReady())`까지만 하고, 실제 `ChangeState(Login)` 호출은 `GameManager`가 구독을 통해 수행. 이 경계를 허물면 "씬 매니저는 전역 시스템을 직접 참조하지 않는다" 원칙이 Logo 케이스부터 깨짐.
- **`GameEvents.cs`에 `LogoSceneReady` 추가 필요**: `plan-eventbus.md`의 이벤트 정의 관례(순수 데이터 레코드)를 따라 `public record LogoSceneReady();`로 추가.
...
- [ ] `LogoSceneManager.cs` 작성 (스플래시 연출 → `LogoSceneReady` 발행)
- [ ] `LoginSceneManager.cs`, `MainMenuSceneManager.cs`, `InGameSceneManager.cs` 스켈레톤 작성
- [ ] `GameEvents.cs`에 `LogoSceneReady` 추가
- [ ] `GameManager.OnAwake()`에 `EventBus.Subscribe<LogoSceneReady>(_ => ChangeState(GameState.Login))` 추가
- [ ] Logo / Login / MainMenu / InGame 씬에 각각 씬 매니저 오브젝트 배치 (`LogoSceneManager`는 영속 `GameManagers`와 별도 오브젝트로 분리 배치)
- [ ] `plan-sceneloader.md`의 `_stateSceneMap`에 `GameState.Logo → "Logo"` 반영 확인
- [ ] 테스트 시나리오 8개 검증
